...
folder_path_x = 'measure/src_x'
folder_path_y = 'measure/src_y'
...
if mode == 'x':
    while True:
        if keyboard.is_pressed('a'):
            if os.path.exists(folder_path_x):
                shutil.rmtree(folder_path_x)
            os.makedirs(folder_path_x)
            ...
            with open('measure/data/data_x.csv', 'w') as f:
                for cnt in result:
    ...
    while True:
        if keyboard.is_pressed('a'):
            if os.path.exists(folder_path_y):
                shutil.rmtree(folder_path_y)
            os.makedirs(folder_path_y)

            result = []

            step = 1
            for i in range(0, 701, step):
                mouse_control.move_mouse_relative(0, step)
                with mss.mss() as sct:
                    monitor = sct.monitors[0]
                    img = np.array(sct.grab(monitor))
                frame = cv2.cvtColor(img, cv2.COLOR_BGRA2BGR)
                ...
                cv2.putText(frame, text, (int(L_cx)+10, int(L_cy)-10), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
                # print(f"x: {L_cx} y: {L_cy} radius: {L_r}")
                result.append([i,L_cy])
                if i % 100 == 0: cv2.imwrite(f"measure/src_y/{i}.jpg", frame)
            ...
            with open('measure/data/data_y.csv', 'w') as f:
                for cnt in result:
                    f.write(f"{cnt[0]},{cnt[1]}\n")
